...
const MovieDisplayCard = ({ allMovies, title }) => {
  ...
  const fixCardListCss = allMovies.length <= 3 ? "" : "justify-content-between";
  ...
  return (
    <>
      ...
      >
        <Container>
          {" "}
          <h2 style={{ color: "crimson" }} className="ps-3 fw-bold pt-2">
            {title}
          </h2>
          ...
          {allMovies.length < 1 ? (
            ...
            <Row className="pb-2 pt-3 ps-2 pe-2">
              <div className="  ps-4 ">
                <Row className={`gap-3 cursor-pointer  p-2 ${fixCardListCss}`}>
                  {allMovies?.map((item) => (
                    <Col
